...
var I18nDict_ja_JP map[uint16]string = map[uint16]string{
	...
	SchematicSupportRemoved:             "最近、一部のユーザーがリセールのためのファイル（schematic形式）をインポートしようとするケースが発生していることに気づきました。クリエイターの創作権利を保護するため、私たちはこのファイル形式のサポートを停止しました。より良い方法を見つけるまで、この機能は回復されません。\n明言しておきますが、FastBuilderは建築物のインポート/エクスポート機能を提供するだけで、リセールはユーザーの個人的な行動であり、FastBuilderとは無関係です。これは、海賊版の本と読者との関係と同じです。\nしかし、私たち（FastBuilder開発チーム、プロジェクトの寄稿者）は、このような事態が続くのを防ぐために全力を尽くす意志があります。",
