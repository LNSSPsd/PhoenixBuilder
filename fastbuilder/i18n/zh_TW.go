...
var I18nDict_zh_TW map[uint16]string = map[uint16]string{
	...
	SchematicSupportRemoved:             "近期，我們注意到，一些使用者嘗試導入倒賣的文件（schematic格式）。出於對創作者的創作權益保護的考量，我們取消了對此文件格式的支援。\n在我們找到更好的保護創作者的創作權益的方式之前，此功能將不會恢復。\n需要聲明的是，FastBuilder功能，倒賣為使用者的個人行為，與FastBuilder無關，正如盜版書籍與閱讀器無關。\n但是，我們（FastBuilder開發團隊、項目貢獻者）願意盡我們所能阻止此類事件繼續發生。",
