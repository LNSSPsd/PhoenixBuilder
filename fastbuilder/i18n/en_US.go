...
var I18nDict_en_US map[uint16]string = map[uint16]string{
	...
	SchematicSupportRemoved:             "Recently, we have noticed that some users are trying to import files (in schematic format) for reselling. To protect the creative rights of the authors, we have discontinued the support for this file format. This feature will not be reinstated until we find a better way to protect the rights of creators.\nIt should be stated that FastBuilder is only responsible for providing the import/export functions for buildings, and resale is a personal action of the users, unrelated to FastBuilder - just like the relationship between pirated books and readers.\nHowever, we (the FastBuilder development team, project contributors) are willing to do our best to prevent such incidents from continuing to happen.",
