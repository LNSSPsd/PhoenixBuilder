...
func (bdump *BDump) writeBlocks(w *bytes.Buffer) error {
	...
	for _, mdl := range bdump.Blocks {
		...
		if mdl.ChestData != nil {
			...
			_, err2 := w.Write([]byte(strings.TrimSpace(dt.Command)))
			_, err3 := w.Write([]byte{0})
			_, err4 := w.Write([]byte(strings.TrimSpace(dt.CustomName)))
			_, err5 := w.Write([]byte{0})
			_, err6 := w.Write([]byte(strings.TrimSpace(dt.LastOutput)))
