...
func (cmd *PlaceCommandBlockWithCommandBlockData) Marshal(writer io.Writer) error {
	...
	_, err = writer.Write(append([]byte(strings.TrimSpace(cmd.CommandBlockData.Command)), 0))
	if err != nil {
		return err
	}
	_, err = writer.Write(append([]byte(strings.TrimSpace(cmd.CommandBlockData.CustomName)), 0))
	if err != nil {
		return err
	}
	_, err = writer.Write(append([]byte(strings.TrimSpace(cmd.CommandBlockData.LastOutput)), 0))
	if err != nil {
		return err
	}
