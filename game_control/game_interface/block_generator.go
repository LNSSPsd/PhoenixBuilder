...
考虑到给定的 pos 可能已经超出了客户端所在维度的高度限制，
因此此函数将会进行自适应处理，
并在返回值 [3]int32 部分告知潜影盒最终生成的位置。

因此潜影盒最终的生成位置可能是非预期的，所以会使用 structure 命令备份一次。
结构的名称将对应返回值 uuid.UUID 参数的字符串形式。
被备份结构包含 1 个方块，对应潜影盒处的原本方块。

另，请使用 GameInterface.RevertStructure 来恢复潜影盒处的方块为原本方块
*/
func (g *GameInterface) PlaceShulkerBox(
	...
	resp := g.SendWSCommandWithResponse("querytarget @s")
	if resp.Error != nil {
		return uuid.UUID{}, [3]int32{}, fmt.Errorf("PlaceShulkerBox: %v", resp.Error)
	}
	got, err := g.ParseTargetQueryingInfo(resp.Respond)
	if err != nil {
	...
	// 如果放置坐标低于了最低限制
	backupBlockPos := [3]int32{pos[0], pos[1], pos[2]}
	switch facing {
	case 0:
	...
	// 备份潜影盒处的方块、清除潜影盒处的方块、修正机器人的朝向、
	// 备份相关的方块、然后再在备份的方块处生成一个绿宝石块。
